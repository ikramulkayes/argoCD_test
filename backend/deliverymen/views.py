...
class ReactView_Register_Deliverymen(APIView):
...
class CheckUserExistenceView_Deliverymen(APIView):
    def post(self, request):
...
class ReactView_DeleteMember_Deliverymen(APIView):
    def post(self, request):
...
class ReactView_Edit_Deliverymen(APIView):
    def post(self, request):
        serializer = ReactSerializer(data=request.data)
        if serializer.is_valid():
