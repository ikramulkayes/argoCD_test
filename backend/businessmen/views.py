...
class ReactView_Register_Businessmen(APIView):
...
class CheckUserExistenceView_Businessmen(APIView):
    def post(self, request):
...
class ReactView_DeleteMember_Businessmen(APIView):
    def post(self, request):
...
class ReactView_Edit_Businessmen(APIView):
    def post(self, request):
        serializer = ReactSerializer(data=request.data)
        if serializer.is_valid():
