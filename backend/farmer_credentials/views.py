...
class ReactView_Register_Farmer(APIView):
...
class CheckUserExistenceView_Farmer(APIView):
    def post(self, request):
...
class ReactView_DeleteMember_Farmer(APIView):
    def post(self, request):
...
class ReactView_Edit_Farmer(APIView):
    def post(self, request):
        serializer = ReactSerializer(data=request.data)
        if serializer.is_valid():
